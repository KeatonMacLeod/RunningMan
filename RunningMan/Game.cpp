...
int const BACKGROUND_APPEAR_TIMER = 3000;
...
SDL_Texture* levelOneText;
SDL_Texture* levelTwoText;
SDL_Texture* levelThreeText;
SDL_Texture* levelFourText;
SDL_Texture* levelFiveText;
...
int levelOneTextureWidth;
int levelOneTextureHeight;
int gemFrameWidth;
int gemFrameHeight;
int gemTextureWidth;
...
{
	SDL_Event event;
	SDL_RenderCopy(renderTarget, title, &cameraRect, NULL);
...
void Game::initSystems()
{
	...
	window = SDL_CreateWindow("Game Engine", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_OPENGL);
	...
	SDL_QueryTexture(levelOneText, NULL, NULL, &levelOneTextureWidth, &levelOneTextureHeight); //textureWidth / Height get values here.
	levelTextRect.w = levelOneTextureWidth;
	levelTextRect.h = levelOneTextureHeight;
	levelTextRect.x = 325;
	levelTextRect.y = 100;
...
void Game::gameLoop()
{
	srand(time(0)); //Makes numbers random by seeding a time since the 1970's XD
...
void Game::processInput()
{
	...
	if (backgroundLevelSet && backgroundLevel == 1)
	{
		SDL_RenderCopy(renderTarget, background, NULL, &backgroundRect);
		SDL_RenderCopy(renderTarget, backgroundFlipped, NULL, &backgroundRectFlipped);
		SDL_RenderCopy(renderTarget, levelOneText, NULL, &levelTextRect);
	...
	if (backgroundLevelSet && backgroundLevel == 2)
	{
		SDL_RenderCopy(renderTarget, background, NULL, &backgroundRect);
		SDL_RenderCopy(renderTarget, backgroundFlipped, NULL, &backgroundRectFlipped);
		SDL_RenderCopy(renderTarget, levelTwoText, NULL, &levelTextRect);
	}
	...
	if (backgroundLevelSet && backgroundLevel == 3)
	{
		SDL_RenderCopy(renderTarget, background, NULL, &backgroundRect);
		SDL_RenderCopy(renderTarget, backgroundFlipped, NULL, &backgroundRectFlipped);
		SDL_RenderCopy(renderTarget, levelThreeText, NULL, &levelTextRect);
	}
	...
	if (backgroundLevelSet && backgroundLevel == 4)
	{
		SDL_RenderCopy(renderTarget, background, NULL, &backgroundRect);
		SDL_RenderCopy(renderTarget, backgroundFlipped, NULL, &backgroundRectFlipped);
		SDL_RenderCopy(renderTarget, levelFourText, NULL, &levelTextRect);
	}
	...
	if (backgroundLevelSet && backgroundLevel == 5)
	{
		SDL_RenderCopy(renderTarget, background, NULL, &backgroundRect);
		SDL_RenderCopy(renderTarget, backgroundFlipped, NULL, &backgroundRectFlipped);
		SDL_RenderCopy(renderTarget, levelFiveText, NULL, &levelTextRect);
	}
...
void Game::loadAllGameTextures()
{
	...
	backgroundFlipped = loadSingleTexture("backgroundFlipped.bmp", renderTarget);
	backgroundScore = loadSingleTexture("backgroundScore.bmp", renderTarget);
	levelOneText = loadSingleTexture("levelOne.bmp", renderTarget);
	levelTwoText = loadSingleTexture("levelTwo.bmp", renderTarget);
	levelThreeText = loadSingleTexture("levelThree.bmp", renderTarget);
	levelFourText = loadSingleTexture("levelFour.bmp", renderTarget);
	levelFiveText = loadSingleTexture("levelFive.bmp", renderTarget);
	...
	brownBlock = loadSingleTexture("brownblock.bmp", renderTarget);
	woodenBlock = loadSingleTexture("woodenblock.bmp", renderTarget);
	yellowBlock = loadSingleTexture("yellowblock.bmp", renderTarget);
	steelBlock = loadSingleTexture("steelblock.bmp", renderTarget);
	diamondBlock = loadSingleTexture("diamondblock.bmp", renderTarget);

	brownBlockBroken = loadSingleTexture("brownblockbroken.bmp", renderTarget);
	woodenBlockBroken = loadSingleTexture("woodenblockbroken.bmp", renderTarget);
	yellowBlockBroken = loadSingleTexture("yellowblockbroken.bmp", renderTarget);
	steelBlockBroken = loadSingleTexture("steelblockbroken.bmp", renderTarget);
	diamondBlockBroken = loadSingleTexture("diamondblockbroken.bmp", renderTarget);

	brownPillar = loadSingleTexture("brownPillar.bmp", renderTarget);
	woodenPillar = loadSingleTexture("woodenPillar.bmp", renderTarget);
	yellowPillar = loadSingleTexture("yellowPillar.bmp", renderTarget);
	steelPillar = loadSingleTexture("steelPillar.bmp", renderTarget);
	diamondPillar = loadSingleTexture("diamondPillar.bmp", renderTarget);
	...
	levelOver = false;
		...
		if (canSpawnBlock)
		{
			if (random < 50)
			{
				...
				Block* block = new Block(woodenBlock, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, blockFrameWidth, blockFrameHeight);
				obstacles.add(block);
			}
			...
			else
			{
				...
				BottomPillar* bottomPillar = new BottomPillar(woodenPillar, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, pillarFrameWidth, pillarFrameHeight);
				obstacles.add(bottomPillar);

				SDL_QueryTexture(woodenPillar, NULL, NULL, &pillarTextureWidth, &pillarTextureHeight);
				pillarFrameWidth = pillarTextureWidth / 1;
				pillarFrameHeight = pillarTextureHeight / 1;
				TopPillar* topPillar = new TopPillar(woodenPillar, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, pillarFrameWidth, pillarFrameHeight);
...
void Game::levelTwo()
{
	...
	if (canSpawnBlock)
	{
		if (random < 50)
		{
			...
			blockFrameWidth = blockTextureWidth / 1;
			blockFrameHeight = blockTextureHeight / 1;
			Block* block = new Block(brownBlock, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, blockTextureWidth, blockTextureHeight);
			obstacles.add(block);
		}
...
void Game::levelFour()
{
	...
	if (canSpawnBlock)
	{
		...
		{
			canSpawnBlock = false;
			SDL_QueryTexture(yellowBlock, NULL, NULL, &blockTextureWidth, &blockTextureHeight);
			blockFrameWidth = blockTextureWidth / 1;
			blockFrameHeight = blockTextureHeight / 1;
			Block* block = new Block(yellowBlock, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, blockTextureWidth, blockTextureHeight);
			obstacles.add(block);
		}
		...
		{
			canSpawnBlock = false;
			SDL_QueryTexture(yellowPillar, NULL, NULL, &pillarTextureWidth, &pillarTextureHeight);
			pillarFrameWidth = pillarTextureWidth / 1;
			pillarFrameHeight = pillarTextureHeight / 1;
			BottomPillar* bottomPillar = new BottomPillar(yellowPillar, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, pillarTextureWidth, pillarTextureHeight);
			bottomPillar->rect.w = pillarFrameWidth;
			bottomPillar->rect.h = pillarFrameHeight;
			obstacles.add(bottomPillar);
			...
			pillarFrameWidth = pillarTextureWidth / 1;
			pillarFrameHeight = pillarTextureHeight / 1;
			TopPillar* topPillar = new TopPillar(yellowPillar, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, pillarTextureWidth, pillarTextureHeight);
			topPillar->rect.w = pillarFrameWidth;
			topPillar->rect.h = pillarFrameHeight;
			obstacles.add(topPillar);
		}
	...
	if (fallingBlockDelayTimer != 0 && fallingBlockDelayTimer % fallingBlockSpawnRate == 0 && !levelOver)
	{
		...
		blockFrameWidth = blockTextureWidth / 1; //Divide the textureWidth by the number of animation columns
		blockFrameHeight = blockTextureHeight / 1; //Divide the textureHeight by the number of animation rows
		FallingBlock* fallingBlock = new FallingBlock(yellowBlockBroken, arrow->currentPosition.x, SCREEN_HEIGHT / 2, blockFrameWidth, blockFrameHeight);
		obstacles.add(fallingBlock);
		arrow = nullptr;
	}
